...
def parse(url: str, art: bool = True, lyrics: bool = True, debugging: bool = False) -> dict or None:
    ...
    for track in tracks:
        if lyrics and track['title_link']:
            track['lyrics'] = get_track_lyrics("{}{}#lyrics".format(artist_url, track['title_link']))
        if track['file'] is not None:
            track = get_track_metadata(track)
            album['tracks'].append(track)

    album['full'] = all_tracks_available(tracks)
    if art:
        album['art'] = get_album_art()
...
def all_tracks_available(tracks) -> bool:
...
def get_album_art() -> str:
    ...
    """
    try:
        url = soup.find(id='tralbumArt').find_all('img')[0]['src']
        return url
    except None:
        pass
